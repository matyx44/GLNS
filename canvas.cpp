...
Canvas::Canvas(std::vector<Arc> arcs, std::vector<Set> sets, Tour tour, int argc, char *argv[]) : scale(10), grab(0, 0), shift(600, 500), arcs(std::move(arcs)), sets(
        std::move(sets)), tour(tour){
    ...
    plannerThread = new std::thread(
            [this, argc, argv]
            {
                planner.entryPoint(this, argc, argv);
            });
...
void Canvas::setArcs(std::vector<Arc> arcs) {
    this->arcs = arcs;
}

void Canvas::setSets(std::vector<Set> sets) {
    this->sets = sets;
}
